...
{
public:
    SqlTreeModelLevel( QSqlQueryModel* model, int parentLevel, int foreignKeyColumn = 1 ) :
        m_model( model ),
...
void SqlTreeModel::appendModel( QSqlQueryModel* model, int foreignKeyColumn )
{
    insertModel( model, d->m_levelData.count() - 1, foreignKeyColumn );
}

void SqlTreeModel::insertModel( QSqlQueryModel* model, int parentLevel, int foreignKeyColumn )
{
    d->m_levelData.append( new SqlTreeModelLevel( model, parentLevel, foreignKeyColumn ) );
    d->m_columns = -1;
}
    ...
    for ( int level = 0; level < d->m_levelData.count(); level++ ) {
        ...
        for ( int row = 0; row < count; row++ ) {
            ...
            if ( levelData->m_parentLevel < 0 ) {
                ...
                int parentId = model->data( model->index( row, levelData->m_foreignKeyColumn ) ).toInt();
